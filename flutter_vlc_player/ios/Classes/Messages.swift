...
/// Generated class from Pigeon that represents data sent in messages.
struct SetSubtitleFontSizeMessage: Hashable {
  var playerId: Int64
  var fontSize: Int64


  // swift-format-ignore: AlwaysUseLowerCamelCase
  static func fromList(_ pigeonVar_list: [Any?]) -> SetSubtitleFontSizeMessage? {
    let playerId = pigeonVar_list[0] as! Int64
    let fontSize = pigeonVar_list[1] as! Int64

    return SetSubtitleFontSizeMessage(
      playerId: playerId,
      fontSize: fontSize
    )
  }
  ...
  static func == (lhs: SetSubtitleFontSizeMessage, rhs: SetSubtitleFontSizeMessage) -> Bool {
    return deepEqualsMessages(lhs.toList(), rhs.toList())  }
  func hash(into hasher: inout Hasher) {
    deepHashMessages(value: toList(), hasher: &hasher)
  }
...
private class MessagesPigeonCodecReader: FlutterStandardReader {
  override func readValue(ofType type: UInt8) -> Any? {
    ...
    case 134:
      return SetSubtitleFontSizeMessage.fromList(self.readValue() as! [Any?])
    default:
...
private class MessagesPigeonCodecWriter: FlutterStandardWriter {
  override func writeValue(_ value: Any) {
    if let value = value as? CreateMessage {
      ...
      super.writeByte(133)
      super.writeValue(value.toList())
    } else if let value = value as? SetSubtitleFontSizeMessage {
      super.writeByte(134)
      super.writeValue(value.toList())
    } else {
...
protocol VlcPlayerApi {
  ...
  func setSubtitleFontSize(msg: SetSubtitleFontSizeMessage) throws
...
class VlcPlayerApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: VlcPlayerApi?, messageChannelSuffix: String = "") {
    ...
    let setSubtitleFontSizeChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.flutter_vlc_player_platform_interface.VlcPlayerApi.setSubtitleFontSize\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      setSubtitleFontSizeChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let msgArg = args[0] as! SetSubtitleFontSizeMessage
        do {
          try api.setSubtitleFontSize(msg: msgArg)
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
        }
      ...
      setSubtitleFontSizeChannel.setMessageHandler(nil)
    }
